...
function checkDependencies() {
  ...
  // Check if chrome-cli is installed, if not, try to install with brew
  try {
    execSync("which chrome-cli");
  } catch {
    console.log("chrome-cli not found. Attempting to install with Homebrew...");
    try {
      execSync("brew install chrome-cli", { stdio: "inherit" });
      // Verify installation
      execSync("which chrome-cli");
      console.log("chrome-cli installed successfully.");
    } catch {
      console.error(
        "Failed to install chrome-cli. Please install it manually: https://github.com/prasmussen/chrome-cli"
      );
      process.exit(1);
    }
  }
...
try {
  ...
  try {
    ...
    // Try to open Chrome if not running
    try {
      execSync("open -a 'Google Chrome'");
      sleep(1000); // Give Chrome time to launch
      execSync("osascript -e 'tell application \"Google Chrome\" to activate'", {
        stdio: "ignore",
      });
    } catch (finalErr) {
      console.error(
        "Unable to activate or launch Chrome browser. Please ensure Chrome is installed and named 'Google Chrome'."
      );
      process.exit(1);
    }
  }
...
function executeScript() {
  const script = async function (selectedText, prompt) {
    ...
    if (selectedText && selectedText.trim() !== "") {
      finalText += `<file_content>${selectedText}</file_content>\n\n${prompt}`;
    } else {
      finalText = prompt;
    }
...
// Open Gemini in the default browser (no auto-fill)
